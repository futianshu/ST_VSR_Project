...
def create_scale_staircase(seq_name, frame_name, lr_coords, scales, output_base_dir, lr_dir, save_path):
    ...
    # 3. 排版绘图：左侧为LR全图（带红框标注裁剪位置），右侧为各倍率切片
    # 右侧小图排成上下两行
...
if __name__ == "__main__":
    # 配置测试参数
    # UDM10 序列名为 3 位数字（000~009），帧名为 4 位数字（0000.png）

    # TARGET_SEQ = "008"
    # TARGET_FRAME = "0000.png"
    # LR_CROP_BOX = (265, 25, 32, 32)

    TARGET_SEQ = "000"
    TARGET_FRAME = "0000.png"
    LR_CROP_BOX = (75, 25, 32, 32)

    TEST_SCALES = [1.5, 2.5, 4.0, 5.5, 8.0]
    # TEST_SCALES = [1.5, 2.5]
    INPUT_SEQ_DIR = f"/home/ubuntu/data/UDM10/BIx4/{TARGET_SEQ}"
    LR_BASE_DIR = "/home/ubuntu/data/UDM10/BIx4"
    OUTPUT_BASE = str(PROJECT_ROOT / "outputs" / "arbitrary_scale_UDM10")
    ...
    SAVE_FILE = os.path.join(OUTPUT_BASE, f"arbitrary_scale_UDM10_{TARGET_SEQ}_{frame_stem}.png")
